...
We might want to examine some summaries of the raster values. We can get a histogram of all the values in a raster just using the `hist()` function:
...
Coming back to our Queensland data, you will remember (or do `st_crs(lgas)`) that it is in the WGS 84 CRS, which is a geographic CRS. We want to transform it into a suitable projected CRS. We will be using GDA2020 / MGA zone 56, which is identified by the EPSG code 7843, and is suitable for SEQ. Transforming between one CRS and another is done using `st_transform()`:

```{r}
lgas_projected <- st_transform(lgas, 7843)
...
The projected raster is in the Geosciences Australia Lambert projection, and the extent, resolution, and dimensions have all changed. This CRS has units of meters, which means the size of each raster cell is `r sprintf("%.2f", res(climate1_projected)[1])`m x `r sprintf("%.2f", res(climate1_projected)[2])`m. 
...
The statistics are similar, but not exactly the same. There are also a lot more NA values in the projected raster, largely because there are more cells: `r format(ncell(climate1), scientific = F)` in the original and `r format(ncell(climate1_projected), scientific = F)` in the projected raster.
